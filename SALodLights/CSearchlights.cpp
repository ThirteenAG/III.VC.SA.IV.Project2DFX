...
{
	if ((CLODLightManager::SA::bRenderOnlyDuringFoggyWeather && *CWeatherFoggyness) || !CLODLightManager::SA::bRenderOnlyDuringFoggyWeather)
	{
		if (CLODLightManager::SA::GetIsTimeInRange(20, 7) && *CLODLightManager::SA::ActiveInterior == 0)
		{
			...
			for (auto it = CLODLightManager::SA::m_pLampposts->cbegin(); it != CLODLightManager::SA::m_pLampposts->cend(); ++it)
			{
				if (it->nDrawSearchlight)
				{
					...
					{
						float fVisibility = CLODLightManager::SA::fSearchlightEffectVisibilityFactor * ((0.0233333f)*sqrt(fDistSqr) - 1.16667f);
...
void __cdecl CSearchlights::drawCustomSpotLightSA(RwV3D StartPoint, RwV3D EndPoint, float TargetRadius, float baseRadius, float slColorFactor1, char slColorFactor2, float slAlpha)
{
	...
	a.z = v108.z;
	CLODLightManager::SA::CVectorNormalize(&a);
	v12 = 0;
	...
	v13 = &TheCamera.GetMatrix()->matrix.pos;
	v110.x = v13->x - StartPoint.x;
	v110.y = v13->y - StartPoint.y;
	v110.z = v13->z - StartPoint.z;
	CLODLightManager::SA::CVectorNormalize(&v110);
	v19 = 0;
	v107 = 0.0f;
	...
	do
	{
		...
		b.y = 0;
		b.z = 1.0f;
		CLODLightManager::SA::CrossProduct(&out, &a, &b);
		CLODLightManager::SA::CVectorNormalize(&out);
		CLODLightManager::SA::CrossProduct(&v132, &out, &a);
		CLODLightManager::SA::CVectorNormalize(&v132);
		...
		if (sqrt(v41 * v41 + v39 * v39 + v38 * v38) > 100.0f)
		{
			...
			v115.y = v40;
			v115.z = v109;
			CLODLightManager::SA::CVectorNormalize(&v115);
			v142 = v115.z * 100.0f;
		...
		CLODLightManager::SA::CVectorNormalize(&v108);
	...
	{
		v89 = 3;
		v53 = CLODLightManager::SA::fSearchlightEffectVisibilityFactor / v107;
		v54 = (unsigned char *)&TempVertexBuffer[1 + CLODLightManager::SA::nSmoothEffect].color;
		do
		{
	...
	{
		v67 = CLODLightManager::SA::fSearchlightEffectVisibilityFactor / v107;
		v90 = (void *)(36 * v52 + (unsigned char *)&TempVertexBuffer[0 + CLODLightManager::SA::nSmoothEffect].color);
		do
		{
	...
	if (v49 >(signed int)v12 && CLODLightManager::SA::RwIm3DTransform(TempVertexBuffer, v51, v12, 0x18u))
	{
		CLODLightManager::SA::RwIm3DRenderIndexedPrimitive(3, TempBufferRenderIndexList, TempBufferIndicesStored);
		CLODLightManager::SA::RwIm3DEnd();
	}
}