...
void CSearchlights::RenderHeliSearchLightsVC()
{
	static CHeliVC ***pPoliceHelis = (CHeliVC ***)0x5AC506;
	CHeliVC* HeliInst = nullptr;
	int nOffset = 0;
		...
		do
		{
			...
			if (HeliInst)
			{
				if (HeliInst->unk > 0.0f)
				{
					...
					{
						EndPoint.z = CLODLightManager::VC::FindGroundZFor3DCoord(HeliInst->shadowPos.x, HeliInst->shadowPos.y, HeliInst->pos.z, 0, 0);
						drawCustomSpotLightVC(HeliInst->pos, EndPoint, 13.0f, 1.0f, 255.0f, 8, 0.8f); //white
					}
				}
			}
			nOffset++;
...
void CSearchlights::RenderSearchLightsVC()
{
	if ((CLODLightManager::VC::bRenderOnlyDuringFoggyWeather && *CWeatherFoggyness) || !CLODLightManager::VC::bRenderOnlyDuringFoggyWeather)
	{
		if (CLODLightManager::VC::GetIsTimeInRange(19, 7))
		{
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEZWRITEENABLE, (void*)0);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEZTESTENABLE, (void*)1);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATESRCBLEND, (void*)2);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEDESTBLEND, (void*)2);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEVERTEXALPHAENABLE, (void*)1);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATETEXTURERASTER, (void*)0);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEFOGENABLE, (void*)0);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATESHADEMODE, (void*)2);
			...
			for (auto it = CLODLightManager::VC::m_pLampposts->cbegin(); it != CLODLightManager::VC::m_pLampposts->cend(); ++it)
			{
				if (it->nDrawSearchlight)
				{
					...
					{
						float fVisibility = CLODLightManager::VC::fSearchlightEffectVisibilityFactor * ((0.0233333f)*sqrt(fDistSqr) - 1.16667f);
						//if (*CWeatherFoggyness)
						//	fVisibility *= 0.5f;

						RwV3D EndPoint = *(RwV3D*)&it->vecPos;
						EndPoint.z = CLODLightManager::VC::FindGroundZFor3DCoord(it->vecPos.x, it->vecPos.y, it->vecPos.z, 0, 0);

						if (!(it->colour.r == 255 && it->colour.g == 255 && it->colour.b == 255))
						{
							//yellow
							drawCustomSpotLightVC(*(RwV3D*)&it->vecPos, EndPoint, min((8.0f * (it->vecPos.z - EndPoint.z)), 90.0f), it->fCustomSizeMult / 6.0f, 5.0f, 8, fVisibility);
						}
						else //if (!(it->colour.r == 254 && it->colour.g == 117 && it->colour.b == 134))
						{
							//white
							drawCustomSpotLightVC(*(RwV3D*)&it->vecPos, EndPoint, min((8.0f * (it->vecPos.z - EndPoint.z)), 90.0f), it->fCustomSizeMult / 6.0f, 255.0f, 8, fVisibility);
						}
						//else
						//{
						//	//pink
			...
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEDESTBLEND, (void*)6);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEVERTEXALPHAENABLE, (void*)0);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATECULLMODE, (void*)2);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEALPHATESTFUNCTION, (void*)5);
			CLODLightManager::VC::RwRenderStateSetVC(rwRENDERSTATEALPHATESTFUNCTIONREF, (void*)2);
		}
	}
}

void __cdecl CSearchlights::drawCustomSpotLightVC(RwV3D StartPoint, RwV3D EndPoint, float TargetRadius, float baseRadius, float slColorFactor1, char slColorFactor2, float slAlpha)
{
	...
	a.z = v108.z;
	CLODLightManager::VC::CVectorNormalize(&a);
	v12 = 0;
	...
	CLODLightManager::VC::CVectorNormalize(&v110);
	v19 = 0;
	v107 = 0.0f;
	...
	do
	{
		...
		b.y = 0;
		b.z = 1.0f;
		CLODLightManager::VC::CrossProduct(&out, &a, &b);
		CLODLightManager::VC::CVectorNormalize(&out);
		CLODLightManager::VC::CrossProduct(&v132, &out, &a);
		CLODLightManager::VC::CVectorNormalize(&v132);
		...
		if (sqrt(v41 * v41 + v39 * v39 + v38 * v38) > 100.0f)
		{
			...
			v115.y = v40;
			v115.z = v109;
			CLODLightManager::VC::CVectorNormalize(&v115);
			v142 = v115.z * 100.0f;
		...
		CLODLightManager::VC::CVectorNormalize(&v108);
	...
	{
		v89 = 3;
		v53 = CLODLightManager::VC::fSearchlightEffectVisibilityFactor / v107;
		v54 = (unsigned char *)&TempVertexBuffer[1 + CLODLightManager::VC::nSmoothEffect].color;
		do
		{
	...
	{
		v67 = CLODLightManager::VC::fSearchlightEffectVisibilityFactor / v107;
		v90 = (void *)(36 * v52 + (unsigned char *)&TempVertexBuffer[0 + CLODLightManager::VC::nSmoothEffect].color);
		do
		{
	...
	if (v49 >(signed int)v12 && CLODLightManager::VC::RwIm3DTransform(TempVertexBuffer, v51, v12, 0x18u))
	{
		CLODLightManager::VC::RwIm3DRenderIndexedPrimitive(3, TempBufferRenderIndexList, TempBufferIndicesStored);
		CLODLightManager::VC::RwIm3DEnd();
	}
}