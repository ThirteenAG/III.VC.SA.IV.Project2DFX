...
export module CoronaLimit;

#define NewLimitExponent 14

export void IncreaseCoronaLimit()
{
    ...
    for (size_t i = dword_temp; i <= (dword_temp + 0x3C); i++)
    {
        auto GoThroughPatterns = [&](const char* pattern_str, int32_t pos) -> void
        {
            ...
            for (size_t j = 0; j < patternl.size(); j++)
            {
                ...
                {
                    injector::AdjustPointer(patternl.get(j).get<uint32_t>(pos), &aCoronas[0], dword_temp, dword_temp + 0x3C);
                    counter1++;
                }
            }
        };
    ...
    for (size_t i = dword_temp; i <= (dword_temp + 0x1B); i++)
    {
        auto GoThroughPatterns = [&](const char* pattern_str, int32_t pos) -> void
        {
            ...
            for (size_t j = 0; j < patternl.size(); j++)
            {
                if (*patternl.get(j).get<uintptr_t>(pos) == i)
                {
                    injector::AdjustPointer(patternl.get(j).get<uint32_t>(pos), &aCoronas2[0], dword_temp, dword_temp + 0x1B);
                    counter2++;
                }
            }
        };
    ...
    injector::AdjustPointer(p.get_first(-4), &aCoronas[0], dword_temp, dword_temp + 0x3C);
    p = hook::pattern("BF FF 05 00 00"); //+
    injector::AdjustPointer(p.get_first(-4), &aCoronas2[0], dword_temp, dword_temp + 0x1B);

    auto pattern = hook::pattern("C1 E1 ? 03 4C 24 18 C1");
    injector::WriteMemory<uint8_t>(pattern.get(0).get<uintptr_t>(2), NewLimitExponent, true);
    ...
    injector::WriteMemory<uint32_t>(pattern.get(0).get<uintptr_t>(2), nCoronasLimit, true);
    pattern = hook::pattern("3D ? ? ? ? 0F 8D ? ? ? ? 8B 44 24 1C F3 0F 10 4C 24");
    injector::WriteMemory<uint32_t>(pattern.get(0).get<uintptr_t>(1), nCoronasLimit, true);
    pattern = hook::pattern("3D ? ? ? ? 72 EC C7 05 ? ? ? ? ? ? ? ? C3");
    injector::WriteMemory<uint32_t>(pattern.get(0).get<uintptr_t>(1), nCoronasLimit * 64, true);
}