...
unsigned short mi_lamppost1;
unsigned short mi_lamppost2;
unsigned short mi_lamppost3;
unsigned short mi_sub_floodlite;
unsigned short mi_Streetlamp1;
unsigned short mi_mlamppost;
unsigned short mi_doublestreetlght1;
unsigned short mi_Streetlamp2;
unsigned short mi_bollardlight;
unsigned short mi_lampost_coast;
unsigned short mi_doc_floodlite;
...
void CLODLightManager::III::RenderSearchLights()
{
	static CPool **pObjectPool = (CPool **)0x880E28;
	CEntityIII *ObjectInst;
	int nObjectPoolSize = (*pObjectPool)->size;

	if (nObjectPoolSize && GetIsTimeInRange(20, 7))
	{
		int nOffset = 0x19C * nObjectPoolSize;
		...
		do
		{
			...
			{
				ObjectInst = (CEntityIII *)(nOffset + (*pObjectPool)->objects);
				if (ObjectInst)
				{
					int IsObjectDamaged = (ObjectInst->m_bRenderDamaged);
					if (!IsObjectDamaged)
					{
						...
						for (auto it = pFileContent->lower_bound(PackKey(ObjectInst->m_nModelIndex, 0)); it != itEnd; it++)
						{
							if (it->second.nDrawSearchlight)
							{
								...
								if (GetDistance(GetCamPos(), &StartPoint) < 1000.0f && GetDistance(&zeroCoords, &StartPoint) > 100.0f)
								{
									...
									{
										//yellow
										drawCustomSpotLight(StartPoint, EndPoint, 8.0f * (StartPoint.z - grZ), it->second.fCustomSizeMult / 6.0f, 5.0f, 8, fSearchlightEffectVisibilityFactor);

									}
									else //if (ObjectInst->m_nModelIndex != GetModelInfoUInt16("Gay_lamppost"))
									{
										//white
										drawCustomSpotLight(StartPoint, EndPoint, 8.0f * (StartPoint.z - grZ), it->second.fCustomSizeMult / 6.0f, 255.0f, 8, fSearchlightEffectVisibilityFactor);
									}
									/*else
									{
									//pink
...
void __cdecl CLODLightManager::III::drawCustomSpotLight(RwV3D StartPoint, RwV3D EndPoint, float TargetRadius, float baseRadius, float slColorFactor1, char slColorFactor2, float slAlpha)
{
	RwMatrix *v12; // ebx@1
	RwV3D *v13; // eax@4
